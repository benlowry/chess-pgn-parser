...
describe('parser.test1.pgn', () => {
  ...
  describe('tokenizeLine', () => {
    it('should tokenize lines', async () => {
      ...
      assert.strictEqual(move1[1], 'd4')
      assert.strictEqual(move1[2], 'Nbd7')
      const move2 = parser.tokenizeLine(tokenizedPGN[2])
      assert.strictEqual(move2.length, 4)
      assert.strictEqual(move2[0], '3.')
      assert.strictEqual(move2[1], 'Bb5')
      assert.strictEqual(move2[2], 'a6')
      assert.strictEqual(move2[3], '{This opening is called the Ruy Lopez.}')
    })
  })
  ...
  describe('parseTurn', () => {
    it('should parse turn', async () => {
      const tokenizedPGN = parser.tokenizeLines(demoPGN)
      const move1 = parser.parseTurn(tokenizedPGN[9])
      assert.strictEqual(move1[0].moveNumber, '10')
      assert.strictEqual(move1[0].to, 'd4')
      assert.strictEqual(move1[0].color, 'w')
      assert.strictEqual(move1[1].moveNumber, '10')
      assert.strictEqual(move1[1].type, 'N')
      assert.strictEqual(move1[1].requireColumn, 'b')
      assert.strictEqual(move1[1].to, 'd7')
      assert.strictEqual(move1[1].color, 'b')
      const move2 = parser.parseTurn(tokenizedPGN[2])
      assert.strictEqual(move2[0].moveNumber, '3')
      assert.strictEqual(move2[0].type, 'B')
      assert.strictEqual(move2[0].to, 'b5')
      assert.strictEqual(move2[0].color, 'w')
      assert.strictEqual(move2[1].moveNumber, '3')
      assert.strictEqual(move2[1].to, 'a6')
      assert.strictEqual(move2[1].color, 'b')
      assert.strictEqual(move2[1].sequence[1], '{This opening is called the Ruy Lopez.}')
    })
  })
