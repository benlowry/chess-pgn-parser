...
describe('parser.test7.pgn', () => {
  const demoPGN = fs.readFileSync(path.join(__dirname, 'parser.test7.pgn')).toString()
  describe('toString', () => {
  ...
  describe('parseTags', () => {
    it('should parse tags', async () => {
      ...
      assert.strictEqual(tags.Event, 'Chinese Chess League')
      assert.strictEqual(tags.Site, 'China CHN')
      assert.strictEqual(tags.Date, '2017.11.04')
      assert.strictEqual(tags.Round, '18.4')
      assert.strictEqual(tags.White, 'Jinshi Bai')
      assert.strictEqual(tags.Black, 'Ding Liren')
      assert.strictEqual(tags.Result, '0-1')
      assert.strictEqual(tags.ECO, 'E21')
      assert.strictEqual(tags.WhiteElo, '2553')
      assert.strictEqual(tags.BlackElo, '2774')
      assert.strictEqual(tags.Annotator, 'Mac')
      assert.strictEqual(tags.PlyCount, '64')
      assert.strictEqual(tags.EventDate, '2017.04.18')
  ...
  describe('tokenizeLine', () => {
    it('should tokenize line', async () => {
      ...
      assert.strictEqual(line1[0], '7...')
      assert.strictEqual(line1[1], 'Nc6')
      const line2 = parser.tokenizeLine(tokenizedPGN[2])
      assert.strictEqual(line2.length, 3)
      assert.strictEqual(line2[0], '3.')
      assert.strictEqual(line2[1], 'Nc3')
      assert.strictEqual(line2[2], "(3.Nf3 {This is a very popular move which can head back towards the Queen's Gambit Declined, or into a new opening, the Queen's Indian.} b6 {[%csl Gb7,Ge4][%cal Gc8b7,Gb7e4] This is the Queen's Indian.} (3...d5 {This is also a good option for black. Now white cannot play the Exchange variation, which is typically seen as one of the best challenges to the QGD.}))")
    })
  })
  ...
  describe('parseTurn', () => {
    it('should parse turn', async () => {
      ...
      assert.strictEqual(turn1[0].type, 'N')
      assert.strictEqual(turn1[0].to, 'c6')
      assert.strictEqual(turn1[0].color, 'b')
      const turn2 = parser.parseTurn(tokenizedPGN[2])
      assert.strictEqual(turn2[0].moveNumber, '3')
      assert.strictEqual(turn2[0].type, 'N')
      assert.strictEqual(turn2[0].to, 'c3')
      assert.strictEqual(turn2[0].color, 'w')
      assert.strictEqual(turn2[0].sequence[2], "(3.Nf3 {This is a very popular move which can head back towards the Queen's Gambit Declined, or into a new opening, the Queen's Indian.} b6 {[%csl Gb7,Ge4][%cal Gc8b7,Gb7e4] This is the Queen's Indian.} (3...d5 {This is also a good option for black. Now white cannot play the Exchange variation, which is typically seen as one of the best challenges to the QGD.}))")
    })
  })
  ...
  describe('processTurn', () => {
    it('should parse turns', async () => {
      ...
      assert.strictEqual(finalPiecePositions.length, 19)
      let found = finalPiecePositions.filter(piece => piece.color === 'b' && piece.type === 'P' && piece.coordinate === 'a7')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'b' && piece.type === 'R' && piece.coordinate === 'f2')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'b' && piece.type === 'R' && piece.coordinate === 'h8')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'b' && piece.type === 'B' && piece.coordinate === 'e7')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'b' && piece.type === 'B' && piece.coordinate === 'g6')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'b' && piece.type === 'N' && piece.coordinate === 'e4')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'b' && piece.type === 'N' && piece.coordinate === 'e5')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'b' && piece.type === 'K' && piece.coordinate === 'h7')
      assert.strictEqual(found.length, 1)

      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'P' && piece.coordinate === 'a2')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'P' && piece.coordinate === 'c4')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'P' && piece.coordinate === 'e3')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'P' && piece.coordinate === 'g2')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'P' && piece.coordinate === 'h3')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'B' && piece.coordinate === 'f1')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'B' && piece.coordinate === 'g5')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'N' && piece.coordinate === 'f7')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'R' && piece.coordinate === 'h1')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'K' && piece.coordinate === 'g4')
      assert.strictEqual(found.length, 1)
      found = finalPiecePositions.filter(piece => piece.color === 'w' && piece.type === 'Q' && piece.coordinate === 'b7')
      assert.strictEqual(found.length, 1)
    })
  })
})
