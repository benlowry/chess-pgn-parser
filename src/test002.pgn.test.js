...
describe('test002.pgn', () => {
  const demoPGN = fs.readFileSync(path.join(__dirname, 'test002.pgn')).toString()
  describe('toString', () => {
  ...
  describe('tokenizeLine', () => {
    it('should tokenize line', async () => {
      ...
      assert.strictEqual(line1[3], 'a5')
      assert.strictEqual(line1[4], '({The point is} 10...Bxd2 11.Qxd2 dxe4 12.Qg5 {[#]})')
      const line2 = parser.tokenizeLine(tokenizedPGN[33])
      assert.strictEqual(line2.length, 2)
      assert.strictEqual(line2[0], '33...')
      assert.strictEqual(line2[1], 'Rd4')
    })
  })
