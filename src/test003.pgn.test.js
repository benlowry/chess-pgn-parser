...
describe('test003.pgn', () => {
  const demoPGN = fs.readFileSync(path.join(__dirname, 'test003.pgn')).toString()
  describe('toString', () => {
  ...
  describe('tokenizeLine', () => {
    it('should tokenize line', async () => {
      ...
      assert.strictEqual(line1[0], '2.')
      assert.strictEqual(line1[1], 'Nf3')
      assert.strictEqual(line1[2], 'Nc6')
      const line2 = parser.tokenizeLine(tokenizedPGN[7])
      assert.strictEqual(line2.length, 7)
      assert.strictEqual(line2[0], '5.')
      assert.strictEqual(line2[1], 'Nxf7')
      assert.strictEqual(line2[2], '$4')
      assert.strictEqual(line2[3], '{[%cal Rf7g5,Rf7h8] After this natural move though, there is no turning back. White went from worse to completely losing with this move.}')
      assert.strictEqual(line2[4], 'Qxg2')
      assert.strictEqual(line2[5], '$1')
      assert.strictEqual(line2[6], '{[%csl Rh1][%cal Rg2h1]}')
    })
  })
  ...
  describe('parseTurn', () => {
    it('should parse turn', async () => {
      const tokenizedPGN = parser.tokenizeLines(demoPGN)
      const turn1 = parser.parseTurn(tokenizedPGN[1])
      assert.strictEqual(turn1.length, 1)
      assert.strictEqual(turn1[0].moveNumber, '1')
      assert.strictEqual(turn1[0].to, 'e5')
      assert.strictEqual(turn1[0].color, 'b')
      const turn2 = parser.parseTurn(tokenizedPGN[5])
      assert.strictEqual(turn2.length, 1)
      assert.strictEqual(turn2[0].moveNumber, '4')
      assert.strictEqual(turn2[0].type, 'N')
      assert.strictEqual(turn2[0].capturing, true)
      assert.strictEqual(turn2[0].to, 'e5')
      assert.strictEqual(turn2[0].color, 'w')
      assert.strictEqual(turn2[0].sequence.length, 5)
      assert.strictEqual(turn2[0].sequence[1], 'Nxe5')
      assert.strictEqual(turn2[0].sequence[2], '$2')
      assert.strictEqual(turn2[0].sequence[3], '{[%csl Re5,Rg2] This unsuspecting move is a costly mistake, allowing Black a double attack.}')
      assert.strictEqual(turn2[0].sequence[4], '(4.Nxd4 $1 exd4 5.O-O {With an advantage})')
    })
  })
